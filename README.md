...
Бот для подбора программ дополнительного образования по каталогу PFDO Мурманской области. Сейчас поддерживает Telegram, MAX, web-chat и Mattermost; Яндекс Алиса подключена как справочный webhook без полного сценария подбора.

## Что уже умеет

- ведет пользователя по короткому onboarding-сценарию;
- показывает единое меню из четырех сценариев: быстрый подбор, пошаговый подбор, углубленная траектория и траектория новых интересов;
- содержит landing page с web-chat, меню сценариев, ссылками на Telegram, MAX и Mattermost;
...
- использует единую скоринговую модель для сценариев 1-4 и отдельно повышает точные совпадения интересов, например баскетбол выше общего спорта;
- строит углубленную траекторию и траекторию новых интересов по 1-5 ссылкам на уже пройденные программы PFDO;
- показывает обзор пройденных программ, классифицированные темы и позволяет изменить критерии поиска перед подбором;
...
Telegram и MAX получают командное меню при старте бота:

- `/start` или `/menu` — главное меню;
- `/text` — подобрать по описанию;
- `/quiz` — подобрать через вопросы;
- `/deep` — составить углубленную траекторию;
- `/wide` — траектория новых интересов;
- `/help` — справка.

Web-chat поддерживает те же сценарии через callback-меню и HTTP API. В Mattermost команды вводятся текстом: `start`, `menu`, `начать` или номер варианта из сообщения бота.

## Landing page

Статический сайт лежит в `landing/`. На странице подключены:

- web-chat с командами `/text`, `/quiz`, `/deep`, `/wide`, `/help`;
- ссылки на Telegram, MAX и Mattermost;
- Яндекс Метрика для аналитики посещений;
- блок статистики по программам PFDO.
...
Основной способ обновить зеркало PFDO и аналитический слой тем:

```bash
npm run pfdo:sync
```

Команда запускает `scripts/sync-pfdo-programs.js`: импортирует каталог, обновляет `pfdo_sync_runs` и `pfdo_program_sync_state`, скачивает документы, извлекает темы и пересобирает аналитику. На сервере этот же сценарий запускается systemd timer `traektoria51-pfdo-sync.timer`.
...
- расширенное логирование и алерты по каналам, PDF и PFDO sync;
- регламент очистки старых сессий, истории рекомендаций и PDF-файлов.
...
## Ограничения текущей версии
 ...
 - реализованы четыре пользовательских сценария, но Алиса пока не использует общий сценарный runtime;
 - сценарии углубленного продолжения и новых интересов используют ссылки PFDO, темы программ и локальное зеркало каталога;
 - если у пройденных программ еще нет классифицированных тем, бот просит пользователя уточнить интересы вручную;
