...
Документ описывает актуальные таблицы локального зеркала PFDO, их назначение, ключи, связи и атрибуты. Структура синхронизирована с `db/pfdo-mirror-schema.sql` на 2026-06-17; значения в колонке `Строк` являются снимком локальной базы и могут отличаться после очередного sync.
